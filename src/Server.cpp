...
// included" (the IRC "trailing parameter" rule) â e.g. `PRIVMSG bob :hi
...
void	Server::handleUSER(Client &client, const std::vector<std::string> &params)
{
	...
	{
		client.appendToWrite("ERROR 451: USER command can only be used by registered clients""\r\n");
		return ;
	}
...
void Server::handlePING(Client &client, const std::vector<std::string> &params)
{
	if (params.empty())
	{
		client.appendToWrite("ERROR 409: PING command requires 1 parameter: <token>""\r\n");
		return;
	}
