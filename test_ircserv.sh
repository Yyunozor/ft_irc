...
# Le nc de macOS (BSD) ferme la socket dès l'EOF sur stdin, souvent avant
# d'avoir lu la réponse du serveur : on garde donc la connexion ouverte
# `timeout` secondes après l'envoi pour laisser le serveur répondre.
send_cmds() {
    local cmds="$1"
    local timeout=${2:-1}
    { echo -e "$cmds"; sleep "$timeout"; } | nc -C $HOST $PORT 2>/dev/null
}
